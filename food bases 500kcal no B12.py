...
for j in range(len(foods)):
	...
	for i in range(1, 283):
		...
		if i in [171, 174, 215, 218, 221, 224, 227, 233, 236, 255, 258, 261, 264, 267, 270, 273, 276, 282]:
...
#bar_chart([foods_single_coverages[i][1] for i in range(foods_on_chart)], [foods_single_coverages[j][2] for j in range(foods_on_chart)], '_Coverage by one food noB12.png', 'staple foods (per 500 kcal) - no B12', range(1, foods_on_chart+1), (13,30))   

two_foods_ranking()
# two foods - top 100
bar_chart([foods_two_coverages[i][2] for i in range(foods_on_chart)], [foods_two_coverages[j][3] for j in range(foods_on_chart)], '_Coverage by two foods - vegan lentils.png', 'two staple foods (500 kcal each) - vegan lentils', range(1, foods_on_chart+1), (13,60))
...
#three_foods_ranking()
# three foods - top 100
#bar_chart([foods_three_coverages[i][3] for i in range(foods_on_chart)], [foods_three_coverages[j][4] for j in range(foods_on_chart)], '_Coverage by three foods - vegan lentils.png', 'three staple foods (500 kcal each) - vegan lentils', range(1, foods_on_chart+1), (13,60))
